show databases;
use my_shop;

create table items3(
item_id int auto_increment primary key,
item_name varchar(50),
item_price int,
item_stock int NOT NULL
);

insert into items3(item_name,item_price,item_stock)values('pajamas','700',8);
desc items3;
select * from items;
select * from items where item_name='hoddie';
select item_price as 'cost of item' from items;
update items set item_price=500 where item_name='t-shirt';
delete from items where item_price=900;
#STRING FUNCTIONS:
select concat(item_name,' ',item_price)  from items;
select concat(item_name,' ',item_price) as item_info  from items;
#SUBSTRING
select substring(item_name,1,5) from items;
#SUBSTRING & CONCAT
select concat(substring(item_name,1,5),'...') from items;
#REPLACE
select replace('hello world','hell','#$%!');
select replace('hello world','l','7');
#Reverse
select reverse('hello');
#Reverse and concat
select concat('woof',' ',reverse('woof'));
#LENGTH
select length('hello');
select upper('hello world');
select lower('INDIA');
Select insert('hello bobby',6,0,' There');
Select left('omghahalol!',3);
select right('omghahalol!!!',5);
select repeat('hahaha',3);
select trim(              'hello'               ) as 'remove blank spaces';
select trim(leading '.' from '...........hello............'               ) as 'remove blank spaces leading';
select trim(trailing  '.' from '...........hello............'               ) as 'remove blank spaces trailing';
select trim(both '.' from '...........hello............'                ) as 'remove blank spaces both sides'
