...
select city from customers where city like '%d';
Select * from books where title like '%\%%'; 
select count(email) from customers;

select count(*) from customers where first_name like '%John%';
...
select max(age),first_name from customers group by first_name order by first_name;
select age from customers where age=(select avg(age) from customers);

SELECT first_name, age FROM customers
WHERE age = (SELECT min(age) FROM customers);

select first_name ,age ,count(*) from customers group by  first_name,age;
